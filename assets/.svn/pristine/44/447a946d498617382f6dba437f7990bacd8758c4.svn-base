...
cc.Class({
    ...
    initialize: function () {
      //  this.spawnCount = DataConfig.kxText.length;
        this.content.height = this.totalCount * (this.itemTemplate.height + this.spacing) + this.spacing; // get total content height
    	for (let i = 0; i < this.spawnCount; ++i) { // spawn items, we only need to do this once
    		let item = cc.instantiate(this.itemTemplate);
    		this.content.addChild(item);
    		item.setPosition(-350, -item.height * (0.5 + i) - this.spacing * (i + 1));
            var obj = DataConfig.kxText[i];
    		//item.getComponent('ListItem').updateItem(obj.id, obj.text);
        ...
        for (let i = 0; i < items.length; ++i) {
            ...
            if (isDown) {
                ...
                if (viewPos.y < -buffer && items[i].y + offset < 0) {
                   ...
                   // let item = items[i].getComponent('ListItem');
                   // let itemId = item.itemID - items.length; // update item id
                    var obj = DataConfig.kxText[i];
                   // item.updateItem(obj.id, obj.text);
                }
            } else {
